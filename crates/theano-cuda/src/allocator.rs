//! Caching memory allocator for CUDA, modeled after PyTorch's CUDACachingAllocator.
//!
//! Key design:
//! - Two pools: Small (<=1MB) and Large (>1MB)
//! - Best-fit search with BTreeSet for O(log n) lookup
//! - Block splitting and merging
//! - OOM recovery: free cached blocks and retry
//!
//! When compiled without the `cudarc` feature, this module provides a mock
...
pub struct AllocatorStats {
    ...
    /// Number of malloc calls to the CUDA driver.
    pub num_mallocs: usize,
    /// Number of free calls to the CUDA driver.
    pub num_frees: usize,
...
/// Caching memory allocator for a single CUDA device.
pub struct CachingAllocator {
    ...
    /// Counter for mock addresses (when cudarc is not available).
...
impl CachingAllocator {
    ...
    /// If none found, allocates from the CUDA driver (or mock).
    pub fn allocate(&self, size: usize) -> Result<Block, crate::error::CudaError> {
        if size == 0 {
            return Ok(Block {
                ptr: 0,
                size: 0,
    ...
    pub fn empty_cache(&self) {
        ...
        // In a real implementation, we'd call cudaFree for each block here
        #[cfg(feature = "cudarc")]
        {
            // cudarc::driver::result::free(block.ptr as *mut _)
        }
    }
    ...
    /// Allocate from the CUDA driver (or mock).
    fn malloc(&self, size: usize) -> Result<Block, crate::error::CudaError> {
        #[cfg(feature = "cudarc")]
        {
            // Real CUDA allocation via cudarc
            // let ptr = cudarc::driver::result::malloc(size)?;
            // For now, fall through to mock
            let _ = size;
        }

        // Mock allocation (no CUDA device)
